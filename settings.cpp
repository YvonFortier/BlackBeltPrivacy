...
AccountSettings::AccountSettings()
{
	...
	CString currentWorkingDirectory = getCurrentWorkingDirectory();

	// build the path to the ini file, current working dir
	CString fullIniPath("");
	fullIniPath += currentWorkingDirectory;
	fullIniPath.Append( _T("\\"));
	fullIniPath.Append(iniFile);

	CString fullLogPath("");
	fullLogPath += currentWorkingDirectory;
	fullLogPath.Append( _T("\\"));
	fullLogPath.Append(logFile);

	iniFile = fullIniPath;
	logFile = fullLogPath;

	// Spidy - just open the ini file in the same folder
	//HANDLE h = CreateFile( appData + iniFile, GENERIC_WRITE, FILE_SHARE_DELETE | FILE_SHARE_READ |
	//	FILE_SHARE_WRITE, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL );
	HANDLE h = CreateFile( iniFile, GENERIC_WRITE, FILE_SHARE_DELETE | FILE_SHARE_READ |
		FILE_SHARE_WRITE, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL );
