...
MessagesContact* MessagesDlg::AddTab(CString number, CString name, BOOL activate, pjsua_call_info *call_info, BOOL notShowWindow, BOOL ifExists)
{
	...
	// Spidy - get the remote port from the remote contact as we need it for calling back...
	CString port;
	if (call_info) {
		CString remoteCaller = PjToStr(&call_info->remote_contact);
		int startLastColon = remoteCaller.ReverseFind(':');
		int startLastBracket = remoteCaller.ReverseFind('>');
		int startLastSemiColon = remoteCaller.ReverseFind(';');
		if (startLastSemiColon > -1) {
			port = (remoteCaller.Mid(startLastColon + 1, startLastSemiColon - startLastColon -1));
		}
		else {
			port = (remoteCaller.Mid(startLastColon + 1, startLastBracket - startLastColon -1));
		}
	}
	// Spidy - get the remote port from the remote contact as we need it for calling back...end.
	...
	// Spidy - remove the port always and add back if we have it.
	// this way a port is always added once only
	if (CString::StringLength(port) > 0) {
		number = (sipuri.user.GetLength() ? sipuri.user + _T("@") : _T("")) + RemovePort(sipuri.domain) + CString(":") + port;
	} else if (sipuri.domain.Find(CString(":")) > 0) {
		number = (sipuri.user.GetLength() ? sipuri.user + _T("@") : _T("")) + sipuri.domain;
	} else {
		// no port found, so append the default port
		number = (sipuri.user.GetLength() ? sipuri.user + _T("@") : _T("")) + sipuri.domain + CString(":5060");
	}
...
#endif
		// Spidy - check for existing tab.
		//if (messagesContact->number == number || compareNumber == number) {
		//if (RemovePort(messagesContact->number) == RemovePort(number) || RemovePort(compareNumber) == RemovePort(number)) {
		if (ExtractHost(messagesContact->number) == ExtractHost(number) || ExtractHost(compareNumber) == ExtractHost(number)) {		
			exists=i;
			if (call_info)
			{
...
// Spidy - check if we are making a call to LAN or WAN
bool MessagesDlg::isLocalURI(pj_str_t pjpj_uri) {
	if (strstr(pjpj_uri.ptr, "localhost")) {
		return true;
	}
	else if (strstr(pjpj_uri.ptr, "LOCALHOST")) {
		return true;
	}
	else if (strstr(pjpj_uri.ptr, "192.168.")) {
		return true;
	}
	else if (!strstr(pjpj_uri.ptr, ".")) {
		// if there is no '.' then we are likely a hostname - so treat as local
		return true;
	}
	else {
		return false;
...
pjsua_call_id MessagesDlg::CallMake(CString number, bool hasVideo, pj_status_t *pStatus)
{
	if (accountSettings.singleMode) {
		...
		// Spidy - do not hold calls. Just a preference from beta testing
	...
	CString s_pj_uri = PjToStr(&pj_uri,0);
	// Spidy - force the transport to the one we are using
	// Spidy - enrich the uri with the transport
	s_pj_uri = GetSIPURI(s_pj_uri, false, isLocalURI(pj_uri));
	pj_uri = StrToPjStr(s_pj_uri);
	...
	// Spidy - if local call then use lan ip otherwise try to use wan ip
	if (isLocalURI(pj_uri)) 
	{
		if (strlen(uPnP::instance()->getLanIPAddress()) > 0) {
	...
	pjsua_acc_config acc_cfg_local;
	pjsua_acc_config_default(&acc_cfg_local);
	pj_pool_t *pool = pjsua_pool_create("acc_cfg_local", 512, 256);
	pjsua_acc_get_config(account, pool, &acc_cfg_local);

	//CString localURI;
	//localURI.Format(_T("<sip:%s@%s:%d>"), accountSettings.account.displayName, accountSettings.account.publicAddr, getRealPort(accountSettings.account));

	//acc_cfg_local.id = pj_str(StrToPj(localURI));
	//acc_cfg_local.force_contact = pj_str(StrToPj(localURI));

	//// Spidy - constrain rtp to the ports we have allocated for it
	//acc_cfg_local.rtp_cfg.port = atoi((CStringA)accountSettings.account.listenPort) + 2;
	//// Spidy - need 2 ports for each call.
	//acc_cfg_local.rtp_cfg.port_range = _GLOBAL_MAX_CALLS_SUPPORTED * 2;
	//// Spidy - configure rtp public address
	acc_cfg_local.rtp_cfg.public_addr = StrToPjStr( accountSettings.account.publicAddr );

	//// Spidy - configure media encryption
	//if (accountSettings.account.srtp ==_T("optional"))
	//{
	//	acc_cfg_local.use_srtp = PJMEDIA_SRTP_OPTIONAL;
	//} else if (accountSettings.account.srtp ==_T("mandatory"))
	//{
	//	acc_cfg_local.use_srtp = PJMEDIA_SRTP_MANDATORY;
	//} else
	//{
	//	acc_cfg_local.use_srtp = PJMEDIA_SRTP_DISABLED;
	//}


	pjsua_acc_modify(acc_id, &acc_cfg_local);
	...
	// Spidy - taking sizeof is only way to check stucture is not empty
	if (sizeof(*pool) > 64) pj_pool_release(pool);
			...
			if (message.GetLength()) {
				...
				if (SendMessage (messagesContact,message) ) {
					...
					if (accountSettings.localDTMF) {
						// Spidy - use current working directory for wav file
						//microsipDlg->RingIn(_T("messageout.wav"),TRUE);
						microsipDlg->RingIn(getCurrentWorkingDirectory() + _T("messageout.wav"),TRUE);
					}
				}
			}
			*pResult= 1;
