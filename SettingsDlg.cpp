...
BOOL SettingsDlg::OnInitDialog()
{
	...
	for (unsigned i=0;i<count;i++)
	{
		if (vid_dev_info[i].fmt_cnt && (vid_dev_info[i].dir==PJMEDIA_DIR_ENCODING || vid_dev_info[i].dir==PJMEDIA_DIR_ENCODING_DECODING))
		{
			...
			// Spidy - use the device name unless its default, in which case use the driver name
			if (vidDevName != "default") {
	...
	CButton *cButtonAutostart;
	cButtonAutostart = (CButton*)GetDlgItem(IDC_AUTOSTART);
	if (regAutoStartExists()) {
		cButtonAutostart->SetCheck(true);
	}
	else {
		cButtonAutostart->SetCheck(false);
		CButton *cButtonStartMinimized;
		cButtonStartMinimized = (CButton*)GetDlgItem(IDC_STARTMINIMIZED);
		cButtonStartMinimized->EnableWindow(false);
	}

	CButton *cButtonStartMinimized;
	cButtonStartMinimized = (CButton*)GetDlgItem(IDC_STARTMINIMIZED);
	if (regAutoStartMinimizedExists()) {
		cButtonStartMinimized->SetCheck(true);
	}
	else {
		cButtonStartMinimized->SetCheck(false);
	}
...
#endif
	ON_BN_CLICKED(IDC_AUTOSTART, &SettingsDlg::OnBnClickedAutostart)
	ON_BN_CLICKED(IDC_STARTMINIMIZED, &SettingsDlg::OnBnClickedStartminimized)
END_MESSAGE_MAP()
...
void SettingsDlg::OnBnClickedAutostart()
{
	regDeleteAutoStart();
	...
	CButton *cButtonAutostart;
	cButtonAutostart = (CButton*)GetDlgItem(IDC_AUTOSTART);
	if (cButtonAutostart->GetCheck()) {
		CButton *cButtonStartMinimized;
		cButtonStartMinimized = (CButton*)GetDlgItem(IDC_STARTMINIMIZED);
		cButtonStartMinimized->EnableWindow(true);

		CButton *cButton;
		cButton = (CButton*)GetDlgItem(IDC_STARTMINIMIZED);
		if (cButton->GetCheck()) {
			// true = minimize
			regSetAutoStart(true);
		}
		else {
			// false = no minimize
			regSetAutoStart(false);
		}
	}
	else {
		CButton *cButton;
...
void SettingsDlg::OnBnClickedStartminimized()
{
	CButton *cbutton;
	cbutton = (CButton*)GetDlgItem(IDC_AUTOSTART);
	if (cbutton->GetCheck()) {
		regDeleteAutoStart();
