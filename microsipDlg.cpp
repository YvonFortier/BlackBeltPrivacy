...
#include "miniupnpc\impl\upnpglobal.hpp"
...
static IMMNotificationClientConcreteClass *m_pDeviceChangeDelegator;
static IMMDeviceEnumerator *m_pDeviceChangeEnumerator;
...
bool PJAccountAndStatusUpdateRunning = false;
static bool timerContactBlinkState = false;
static bool beepPlaying = false;
static CString balanceURL;

const UINT MYWM_SHELL_TRAY_MSG = RegisterWindowMessageA("TaskbarCreated");

void IMMNotificationClientConcreteClass::initialize()
{
	...
	HRESULT hr = CoCreateInstance(
		 ...
		 (LPVOID*)&m_pDeviceChangeEnumerator);
}

STDMETHODIMP IMMNotificationClientConcreteClass::OnDeviceAdded(LPCWSTR pwstrDeviceId) {
	if (IsWindow(microsipDlg->m_hWnd)) {
		microsipDlg->OnDeviceChange();
	}

	return S_OK;
}

STDMETHODIMP IMMNotificationClientConcreteClass::OnDefaultDeviceChanged(EDataFlow flow, ERole role, LPCWSTR pwstrDefaultDeviceId) {
	if (IsWindow(microsipDlg->m_hWnd)) {
		microsipDlg->OnDeviceChange();
	}
	...
	case PJSIP_INV_STATE_DISCONNECTED:
		...
		if (call_info->last_status == 200)
		{
			...
			if (accountSettings.localDTMF) {
				// spidy - use current folder for wav file location
				//microsipDlg->RingIn(_T("hangup.wav"), TRUE, TRUE);
				microsipDlg->RingIn(getCurrentWorkingDirectory() + _T("hangup.wav"), TRUE, TRUE);
			}
		} else {
			...
			CString strCallReport;
			strCallReport = PjToStr(&call_info->last_status_text).GetBuffer();
			// Spidy - incoming call was missed, switch to calls view as a hint to user
			if ((strCallReport == _T("Request Terminated")) && (call_info_t.role == PJSIP_ROLE_UAS)) {
				microsipDlg->GotoTab(1);
			}
							 ...
							 pjsip_rx_data *rdata)
{
	...
	if (IsWindow(microsipDlg->m_hWnd)) {
		...
		if (autoAnswer!=1 || pjsua_call_get_count()>1)
		{
			if (accountSettings.hidden) {
				...
				if (call_get_count_noincoming()) {
					...
					// Spidy - use current working directory for ringin if no path specified
					if (!accountSettings.ringingSound.GetLength()) {
						microsipDlg->RingIn(getCurrentWorkingDirectory() + _T("ringin.wav"));
					} else if (accountSettings.ringingSound.Find(CString(":\\"), 0) == -1) {
						microsipDlg->RingIn(getCurrentWorkingDirectory() + accountSettings.ringingSound);
					} else {
		...
		if (playBeep) {
			microsipDlg->RingIn(getCurrentWorkingDirectory() + _T("ringin2.wav"), TRUE, TRUE);
		}
	}
...
{
	// Spidy - use on_pager2 to get the port out of the message for use in adding a new msg tab
	pj_str_t fromWithPort;
	char strMsgPort[10];
	CString cstrMsgPort;
	cstrMsgPort.Empty();
	itoa (rdata->msg_info.via->sent_by.port,strMsgPort,sizeof(strMsgPort));
	cstrMsgPort.Append(CString(strMsgPort));
	fromWithPort = StrToPjStr(AddPortToSipURI(PjToStr(from), cstrMsgPort)); 
	if (IsWindow(microsipDlg->m_hWnd))
	{
		microsipDlg->SendMessage(MYWM_ON_PAGER, (WPARAM)&fromWithPort, (LPARAM)body);
	}
}
...
CmicrosipDlg::~CmicrosipDlg(void)
{
	...
	m_pDeviceChangeEnumerator->UnregisterEndpointNotificationCallback(m_pDeviceChangeDelegator);
    m_pDeviceChangeDelegator->OnPFooFinalRelease();
    m_pDeviceChangeDelegator->Release();
}
...
BEGIN_MESSAGE_MAP(CmicrosipDlg, CBaseDialog)
	...
	ON_REGISTERED_MESSAGE(MYWM_SHELL_TRAY_MSG, OnExplorerCrashTrayRecover)	ON_MESSAGE(MYWM_NOTIFYICON,onTrayNotify)
...
void CmicrosipDlg::CreateTrayIcon() {
...
BOOL CmicrosipDlg::OnInitDialog()
{
	...
	m_pDeviceChangeDelegator = new(IMMNotificationClientConcreteClass);
	m_pDeviceChangeDelegator->initialize();
	HRESULT result = m_pDeviceChangeEnumerator->RegisterEndpointNotificationCallback(m_pDeviceChangeDelegator);
	...
	// Spidy - set timers for account and presence updates
	SetTimer(IDT_TIMER_UPDATE_ACCOUNTS,4999,NULL);
	SetTimer(IDT_TIMER_UPDATE_PRESENCE,59999,NULL);
...
void CmicrosipDlg::OnMenuAccountChange(UINT nID)
{
	...
	// Spidy - grab the port for updating later
	OldNewUpnpSpec oldNewUpnpSpec;
	oldNewUpnpSpec.oldPort =  getRealPort(accountSettings.account);
	...
	if (accountSettings.accountId) {
		...
		pageContacts->PresenceUnsubscribe();
		PJResetStack();
...
{
	// Spidy - clean up upnp - delete known entries.
	if (strlen(uPnP::instance()->getExternalIPAddress()) > 0) {
		UINT uniqueID = rand();
		systray_shutdown_notify(microsipDlg->m_hWnd, uniqueID);
		// Spidy - minimize the window while we cleanup upnp etc.
		this->ShowWindow(SW_HIDE);
		this->UpdateWindow();
		uPnP::instance()->disableAllPorts(NULL);
		systray_del(microsipDlg->m_hWnd, uniqueID);
	}
...
LRESULT CmicrosipDlg::onPowerBroadcast(WPARAM wParam,LPARAM lParam)
{
	if (wParam == PBT_APMRESUMEAUTOMATIC)
	{
		...
		// Spidy - start the uPnP subsystem to get ipaddresses and update ports.
		uPnP::instance();
		// Spidy - sleep to give enough time to compute ipaddresses
		Sleep(250);
		...
		// Spidy - unsubscribe here to then resubscribe much later 
		// Spidy - when ip address computed and ports open hopefully
		//pageContacts->PresenceUnsubscribe();
...
void CmicrosipDlg::PJUpdatePresence () {
	//pageContacts->PresenceUnsubscribe();

	//pjsua_pres_shutdown(NULL);
	//if (account > PJSUA_INVALID_ID) {
	//	if (pjsua_acc_is_valid(account)) {
	//		if (pj_ready) {
	//			pjsua_pres_init_acc(account);
	//		}
	//	}
	//}

	pageContacts->PresenceSubscribe();
	pageContacts->PresenceUpdate();
	PublishStatus();
}	
...
void CmicrosipDlg::OnTimer (UINT TimerVal)
{
	...
	// Spidy - add a timer response to update presence of contacts
	if (TimerVal == IDT_TIMER_UPDATE_PRESENCE) {
		// Spidy - may have to subscribe if account was switched for example.
		if (pjsua_call_get_count() == 0) {
			if ((!(PJAccountAndStatusUpdateRunning || pageContacts->presenceOperationIsActive) && pj_ready)) {
				PJUpdatePresence();
			}
		}
	...
	// Spidy - add a timer response to update the account with ip and update status
	else if (TimerVal == IDT_TIMER_UPDATE_ACCOUNTS) {
		if (pjsua_call_get_count() == 0) {
			if ((!(PJAccountAndStatusUpdateRunning || pageContacts->presenceOperationIsActive)) && pj_ready) {
				PJAccountAndStatusUpdate();
			}
		}
		...
		if (TimerVal == IDT_TIMER_0) {
			...
			microsipDlg->RingIn(getCurrentWorkingDirectory() + _T("ringout.wav"), TRUE, TRUE);
		}
}
...
#endif
#ifndef _GLOBAL_NO_MESSAGING
	// Spidy - change to on_pager2 for more information
//	ua_cfg.cb.on_pager = &on_pager;
	...
	if (accountSettings.enableLog) {
		...
		// Spidy - release the buffer when we are sure its been finished with
		buf.ReleaseBuffer();
	} else {
...
void CmicrosipDlg::PJAccountAdd()
{
	...
	// Spidy - add rtp configuration
	// constrain rtp to the ports we have allocated for it
	acc_cfg.rtp_cfg.port = getRealPort(accountSettings.account) + 2;
	// need 2 ports for each call.
	acc_cfg.rtp_cfg.port_range = _GLOBAL_MAX_CALLS_SUPPORTED * 2;

	if (strlen(uPnP::instance()->getExternalIPAddress()) > 0) {
		accountSettings.account.publicAddr.SetString(CString(uPnP::instance()->getExternalIPAddress()));
	} 
	else if (strlen(uPnP::instance()->getLanIPAddress()) > 0) {
		accountSettings.account.publicAddr.SetString(CString(uPnP::instance()->getLanIPAddress()));
	} else {
		accountSettings.account.publicAddr.SetString(CString("127.0.0.1"));
	}

	// Spidy - configure media encryption
	if (accountSettings.account.srtp ==_T("optional"))
	{
	...
	// Spidy - set the callee port into the contact parameters to be picked up the other side of the call
	CString contactParams = PjToStr(&acc_cfg.contact_params);
	contactParams.Append(CString(";calleePort="));
	contactParams.Append(accountSettings.account.listenPort);
	acc_cfg.contact_params = pj_str(StrToPj(contactParams));
...
#endif
	// Spidy - use a fuller uri
	//localURI.Format(_T("\"%s\" <sip:%s@%s:%d>"), accountSettings.account.displayName , accountSettings.account.username, accountSettings.account.publicAddr, getRealPort(accountSettings.account));
	localURI.Format(_T("<sip:%s@%s:%d>"), accountSettings.account.username, accountSettings.account.publicAddr, getRealPort(accountSettings.account));
	//localURI.Format(_T("<sip:%s@%s>"), accountSettings.account.username, accountSettings.account.publicAddr);

	acc_cfg.id = StrToPjStr(localURI);
	acc_cfg.force_contact = StrToPjStr(localURI);
	...
	if (status == PJ_SUCCESS) {
		// Spidy - set the listen port for upnp networking
		int newPort = getRealPort(accountSettings.account);
		uPnP::instance()->updateAllPortsMultiThreaded(0, newPort, 6);
	}

	// Spidy - handle presence now the account is loaded.
	pageContacts->PresenceSubscribe();
	pageContacts->PresenceUpdate();
	PublishStatus();
}

void CmicrosipDlg::PJResetStack () {
...
// Spidy - update account, status and presence as net env we are in changes or user account changes
void CmicrosipDlg::PJAccountAndStatusUpdate()
{
	// Spidy - only if no presence op is running and no conflicting windows are open
	if (pageContacts->presenceOperationIsActive || ::IsWindowVisible(pageContacts->addDlg->m_hWnd) || accountDlgIsActive || !pj_ready) {
		return;
	}

	PJAccountAndStatusUpdateRunning = true;
	// Spidy - this timer thread potentially re-initializes pjsua. Care must be taken in its execution

	// Spidy - make sure we are not in call
	if (pjsua_call_get_count() == 0) {
		// account config
		if (account != PJSUA_INVALID_ID) { 
			if (pjsua_acc_is_valid(account)) {
				// copy the current account config to the local one here
				pjsua_acc_config acc_cfg_test_ip;
				pjsua_acc_config_default(&acc_cfg_test_ip);

				CString calculatedAddressColonPort;
				if (strlen(uPnP::instance()->getExternalIPAddress()) > 0) {
					calculatedAddressColonPort.Append(CString(uPnP::instance()->getExternalIPAddress()));
					calculatedAddressColonPort.Append(CString(":"));
					calculatedAddressColonPort.Append(CString(accountSettings.account.listenPort));
				} 
				else if (strlen(uPnP::instance()->getLanIPAddress()) > 0) {
					calculatedAddressColonPort.Append(CString(uPnP::instance()->getLanIPAddress()));
					calculatedAddressColonPort.Append(CString(":"));
					calculatedAddressColonPort.Append(CString(accountSettings.account.listenPort));
				}
				else {
					calculatedAddressColonPort.Append(CString("127.0.0.1"));
					calculatedAddressColonPort.Append(CString(":"));
					calculatedAddressColonPort.Append(CString(accountSettings.account.listenPort));
				}

				CString accountAddressColonPort;
				accountAddressColonPort.Format(_T("%s:%s"), accountSettings.account.publicAddr, accountSettings.account.listenPort);

				// if address changed, update the pjsua stack
				if (strstr(StrToPjStr(calculatedAddressColonPort).ptr, StrToPjStr(accountAddressColonPort).ptr) == NULL) {
					PublishStatus(false, false);
					pageContacts->PresenceUnsubscribe();
					PJResetStack();
					if (accountSettings.accountId) {
				...
				////if (strstr(StrToPjStr(accountSettings.account.publicAddr).ptr, externalIPAddress) == NULL) {
				//if ( (strcmp(externalIPAddressOld, externalIPAddress) != 0) || (strcmp(lanIPAddressOld, lanIPAddress) != 0) ) {
				//	PublishStatus(false, false);
				//	pageContacts->PresenceUnsubscribe();
				//	PJResetStack();
				//	if (accountSettings.accountId) {
		...
		if ((strlen(uPnP::instance()->getExternalIPAddress()) > 0) && pj_ready) {
			// Spidy - lets set status to ready so we know we can now make calls.
			UpdateWindowText(Translate(_T("Internet Ready")), IDI_ONLINE);
		}
		else if ((strlen(uPnP::instance()->getLanIPAddress()) > 0) && pj_ready) {
			// Spidy - lets set status to ready so we know we can now make calls.
			UpdateWindowText(Translate(_T("LAN Ready")), IDI_ONLINE);
		}
		else {
			// Spidy - lets set status to not ready so we know we cannot make calls.
			UpdateWindowText(Translate(_T("Not Ready")), IDI_OFFLINE);
		}
	}

	PJAccountAndStatusUpdateRunning = false;
}

void CmicrosipDlg::PJAccountAddLocal()
{
	// dont add a local account. It just messes everything up
	return;
...
#endif
	// Spidy - make sure the account is valid as well
	if (account != PJSUA_INVALID_ID) {
		if (pjsua_acc_is_valid(account)) {
		...
		if (pjsua_acc_is_valid(account))
		{
			...
			if ( str != _T("Default status message") ) {
				...
				if (noReg || str==_T("OK"))
				{
					...
					// Spidy - dont append 'outgoing' because its confusing
					//if (noReg) {
...
#ifndef _GLOBAL_NO_CONTACTS
					// Spidy - dont subscribe here, just do an update...
					//pageContacts->PresenceSubscribe();
					pageContacts->PresenceUpdate();
#endif
...
void CmicrosipDlg::PublishStatus(bool online, bool init)
{
	...
	if (account > PJSUA_INVALID_ID) {
		if (pjsua_acc_is_valid(account)) // account
		{
			pjrpid_element pr;
			pr.type = PJRPID_ELEMENT_TYPE_PERSON;
			pr.id = pj_str(NULL);
...
void CmicrosipDlg::DialNumberFromCommandLine(CString number) {
	...
	if (pj_ready && (
		...
		// Spidy - let not care about info.status==200. We have a valid account and number - lets dial it.
		//(accountSettings.accountId>0 && pjsua_acc_is_valid(account) && pjsua_acc_get_info(account,&info)==PJ_SUCCESS && info.status==200)
		(accountSettings.accountId>0 && pjsua_acc_is_valid(account) && pjsua_acc_get_info(account,&info)==PJ_SUCCESS)
		) ) {
			DialNumber(number);
	} else {
...
void CmicrosipDlg::RingIn(CString filename, BOOL noLoop, BOOL inCall)
{
	...
	if (!filename.IsEmpty()) {
		// Spidy - use current working directory
		//beepPlaying = filename == _T("ringin2.wav");
		beepPlaying = filename == getCurrentWorkingDirectory() + _T("ringin2.wav");
...
{
	// Spidy - update status to offline
	PublishStatus(false, false);
...
BOOL CmicrosipDlg::OnDeviceChange(void) {
	if (pj_ready) {
		// register this other thread...
		pj_thread_desc thread_desc;
		pj_bzero(&thread_desc, sizeof(pj_thread_desc));
		pj_thread_t *thread = 0;
		pj_status_t status;
		status = pj_thread_register("OnDeviceChange", thread_desc, &thread);							
		// register this other thread...end.
...
int getRealPort (Account& oAccount) {
	if (!oAccount.listenPort.CompareNoCase(CString("Auto")) || !accountSettings.account.listenPort.CompareNoCase(CString(""))) {
		if (!oAccount.transport.CompareNoCase(CString("TLS"))) {
			return 5060 +1;
		}
		else {
			return 5060;
	...
	else if (!oAccount.listenPort.CompareNoCase(CString(""))) {
		return 0;
	}
	else {
		if (!accountSettings.account.transport.CompareNoCase(CString("TLS"))) {
			return atoi((CStringA)oAccount.listenPort) + 1;
		}
		else {
			return atoi((CStringA)oAccount.listenPort);
		}
	}
}

HRESULT CmicrosipDlg::OnExplorerCrashTrayRecover(WPARAM wParam_i, LPARAM lParam) {
	CreateTrayIcon();
	return 0;
}