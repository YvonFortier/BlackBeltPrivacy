...
Preview::Preview(CWnd* pParent)
{
	...
	CString strClass = AfxRegisterWndClass(
		...
		// Spidy - put the preview window in the taskbar. Beta test preference
...
void Preview::Start(int id)
{
	...
	// Spidy - set the window position to be bottom right
	//SetWindowPos(NULL, x, y, w, h, SWP_NOZORDER|SWP_SHOWWINDOW);
	RECT WorkArea;
	SystemParametersInfo(SPI_GETWORKAREA, 0, &WorkArea, 0);
	//SetWindowPos(NULL, x, y, w, h, SWP_NOZORDER|SWP_SHOWWINDOW);
	SetWindowPos(NULL, WorkArea.right - 320 - 10, WorkArea.bottom - 240 -10, 320, 240, SWP_SHOWWINDOW);
