...
CString ExtractHost(CString uri)
{
	// Spidy - extracts the host, whether name or ip from a sip uri
	CString uriWithoutPort;
	int pos = uri.Find(_T(":"));
	if (pos != -1) {
		uriWithoutPort = uri.Mid(0,pos);
	}

	CString ip;
	pos = uriWithoutPort.Find(_T("@"));
	if (pos > -1) {
		ip = uriWithoutPort.Mid(pos + 1, CString::StringLength(uriWithoutPort) - pos);
	} else {
		ip = uriWithoutPort;
	}
...
CString AddPortToSipURI(CString sipuri, CString port)
{
	int pos = sipuri.ReverseFind(_T('>'));
	if (pos != -1) {
		sipuri.Insert(pos, CString(":") + port);
		return sipuri;
	} else {
		return sipuri;
	}
...
bool isLANip(CString ip) {
	if (ip.Find(CString("localhost"),0) > 0) {
		return true;
	}
	else if (ip.Find(CString("LOCALHOST"),0) > 0) {
		return true;
	}
	else if (ip.Find(CString("192.168."), 0) > 0) {
		return true;
	}
	else if (ip.Find(CString("."), 0) == 0) {
		return true;
	}
	else {
		return false;
...
CString getCurrentWorkingDirectory(void) {
	// Spidy - get current working directory
	LPTSTR ptr;
	CString currentWorkingDirectory("");
	currentWorkingDirectory.Preallocate(MAX_PATH);
	ptr = currentWorkingDirectory.GetBuffer();
	GetModuleFileName(NULL, ptr, MAX_PATH);
	currentWorkingDirectory.ReleaseBuffer();

	// remove the file name from the end of the directory
	currentWorkingDirectory.Delete(currentWorkingDirectory.ReverseFind('\\'), CString::StringLength(currentWorkingDirectory) - currentWorkingDirectory.ReverseFind('\\'));

	// append the final '\' to make it a full path
	currentWorkingDirectory.Append(_T("\\"));
